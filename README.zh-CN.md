...
![tcomp 在终端中并排对比两份文档](assets/screenshot.png)
...
- 支持拖动中间分割线、默认开启的长行换行展示（可在 **Edit** 菜单切换）、可选的左右面板独立横向滚动、闪烁光标、顶部菜单和底部命令栏。
...
第一行包含 `File`、`Edit`、`Compare`、`Help` 菜单，右上角显示颜色图例。**Edit → ✓ Wrap Line** 默认已勾选：同一对比行会共同展开，以保持左右面板对齐；取消勾选后，每行保持在一个屏幕行内，并可使用横向滚动。最底行提供可点击的 `Help`、`Reset`、`Replace`、`Match`、`Copy`、`Quit`，状态和错误信息显示在它的上一行。
...
- 普通滚轮同步纵向滚动两侧。
- 取消 **Edit → Wrap Line** 勾选后，横向滚轮只滚动指针所在面板；终端不发送横向滚轮事件时，可以按住 `Shift` 使用普通滚轮。边缘的 `‹` / `›` 表示仍有隐藏内容，状态栏显示每侧当前可见列范围。
- 顶部菜单和底部命令都可以直接点击。
...
`Ctrl+C` 会退出 `tcomp`。使用 **Edit → Copy selection** 或选中文本后的右键菜单可复制到 `tcomp` 内部剪贴板，并通过 OSC 52 请求终端写入系统剪贴板。`Ctrl+X` 会剪切并复制，`Ctrl+V` 读取内部剪贴板。终端程序无法跨平台直接读取系统剪贴板，因此外部粘贴请使用终端提供的 `Shift+Insert`、`Ctrl+Shift+V` 或右键粘贴命令。
...
| Ctrl+A | 选择当前侧全部文本 |
| Ctrl+X / Ctrl+V | 剪切 / 内部粘贴 |
| Copy selection | Edit 菜单或选中文本后的右键菜单 |
